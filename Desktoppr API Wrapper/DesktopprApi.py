...
class DesktopprAPI:
    '''This class allows you to create an object that allows you to query the
    desktoppr site using their public api.
    '''
    ...
    def authorize_user_pass(self, username, password):
        '''Gets a privileged access key by authorizing to the site with a
        username/password. Stores the users API key for further privileged
        access in this session.
        '''
        r = requests.get('https://api.desktoppr.co/1/user/whoami',
    ...
    def get_user_info(self, username):
        '''Returns a dictionary describing a specific user.'''
    ...
    def get_wallpapers_URL(self, page=1, safefilter='safe'):
        '''This is a subset of get_wallpapers(), which returns a page of wallpaper URLs.
        The API does not document sorting options.
        '''
        if safefilter != 'safe' and safefilter != 'include_pending' and safefilter != 'all':
            print('Unknown filter:',safefilter,'Valid options are safe, include_pending, all')
            return
    ...
    def follow_user(self, username, unfollow=False):
        ...
        '''
        if self.apikey == None:
            print('ERROR: This is a user command. You must first authenticate as a user with authorize_user_pass() or authorize_API() method.')
            return
        r = None
        if unfollow:
            ...
            r = requests.delete(self.baseurl +'users/' + username + '/follow',
